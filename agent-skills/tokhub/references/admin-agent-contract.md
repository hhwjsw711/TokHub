...
Every non-read request, export, and download must include:
...
For local setup, `scripts/tokhub.mjs admin-agent bootstrap --url https://host` may perform the owner browser-session-equivalent login flow by prompting for username/email and password in the terminal, then creating a short-lived admin-agent token. Do not ask the user to paste the password or resulting token into chat.
...
`scripts/tokhub.mjs` redacts known plaintext key fields from JSON stdout and JSON `--output` files. Artifact responses, including audit exports, usage exports, platform channel CSV exports, and channel-site packages, require explicit `--output`. Output files are written with `0600`, refuse overwrite without `--overwrite`, and are blocked inside the current git worktree unless `--allow-repo-output` is passed. Treat secret-bearing files as key material.
